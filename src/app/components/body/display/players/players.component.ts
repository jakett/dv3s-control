...
export class PlayersComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.player1['active_pane_idx'] = 0;
    ...
    this.player2['active_pane_idx'] = 1;
  ...
  onActivePane(object): void {
    const playerId = object.player_id;
    const idx = this.players.findIndex(player => player.id === playerId);

    this.players[idx].active_pane_idx = object.pane_idx;
  }
  ...
  activePaneFn(idx) {
    this.activePaneIdx = idx;
    console.log('IIIIII go to activePaneFn, activePaneIdx = ' + this.activePaneIdx);
  }
}
